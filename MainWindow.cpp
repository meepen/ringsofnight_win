...
public:
	ViewWin(int width, int height) {
		...
		hwnd_ = CreateWindowW(szWindowClass,
			szTitle,
			WS_OVERLAPPEDWINDOW,
			CW_USEDEFAULT,
			CW_USEDEFAULT,
	...
	virtual void OnChangeTitle(Awesomium::WebView* caller,
		...
		SetWindowTextW(hwnd_, title_wide.c_str());
	}
	...
	{
		auto mlen = message.length();
		char *msg = new char[mlen + 1];
		message.ToUTF8(msg, mlen);
		msg[mlen] = 0;

		auto slen = source.length();
		char *src = new char[slen + 1];
		source.ToUTF8(src, slen);
		src[slen] = 0;
